...
export default function FloatingCart() {
  try {
    ...
    useEffect(() => {
      setIsMounted(true);
      console.log('FloatingCart mounted, cartCount:', cartCount);
    }, []);
    ...
    useEffect(() => {
      if (cartCount > (prevCartCount || 0)) {
        ...
        console.log('Item added to cart, count:', cartCount);
        setTimeout(() => setShowNotification(false), 3000);
      }
    }, [cartCount, prevCartCount]);

    // Always render the button structure, but show/hide based on cart
    if (!isMounted) {
      return (
    ...
    console.error('FloatingCart Error:', error);
    return null;
  }
}
